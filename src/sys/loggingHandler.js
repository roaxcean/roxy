 ...
 */
async function canSendTo(channelId) {
    try {
        ...
        if (!me)
            return false;
        const perms = BigInt(channel.permissionsOf(me).allow ?? 0);
...
async function sendOne(channelId, payload) {
...
function bufferFor(channelId) {
    if (!pendingBuffers.has(channelId))
        pendingBuffers.set(channelId, []);
