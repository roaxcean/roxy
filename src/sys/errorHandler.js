...
function isIgnored(err) {
    ...
    const code = err?.code ?? "";
    return (IGNORED_MESSAGES.some(s => msg.startsWith(s)) ||
...
export function formatError(err) {
    ...
    const rawStack = e?.stack ?? String(err);
    const lines = rawStack.split("\n");
    const capped = isFinite(MAX_STACK_LINES) && lines.length > MAX_STACK_LINES + 2
        ? [
 ...
 */
async function handleError(err, guildId) {
    if (isIgnored(err))
        return false;
    const e = err;
    if (e?.message === "Disallowed intents specified") {
        consola.error("Disallowed intents specified\n|\n" +
