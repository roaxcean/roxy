 ...
 */
export async function isStaff(userId, roleIds = []) {
    const cfg = await getAppConfig();
    if (cfg.staff.userIds.includes(userId))
        return true;
    if (cfg.staff.roleIds.some(r => roleIds.includes(r)))
        return true;
    return false;
}
 ...
 */
export async function buildPingLine(guildId) {
    const cfg = await getAppConfig();
    // Try guild-specific first, then global fallback
    const target = (guildId ? cfg.pingTargets.find(p => p.guildId === guildId) : undefined) ??
        cfg.pingTargets.find(p => !p.guildId);
    if (!target)
        return "> No pings configured.";
    const parts = [
