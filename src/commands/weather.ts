...
function icon(code: string) { return WMO_ICONS[code] ?? "<:temperaturelow:1477304884876279878>"; }
...
export default {
    ...
    async function(interaction: CommandInteraction) {
        const location = getOption<string>(interaction, "location")!;
        const units    = getOption<string>(interaction, "units") ?? "metric";
        ...
        async function fetchWttr(): Promise<any> {
        ...
        try {
            ...
            await MessageHandler.error(
                interaction,
                isTimeout ? "wttr.in did not respond in time." : (err?.message ?? String(err)),
                "Could not fetch weather data. Check your location spelling and try again."
            );
            return;
        }
