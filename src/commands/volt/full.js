...
export default {
    ...
    function: async (int) => {
        ...
        const nexthumbnail = `https://api.synkradio.co.uk/spotify/cover?format=webp&width=576&height=576&artist=${data.playing_next.song.artist.split(";")[0].replaceAll(" ", "%20")}&title=${data.playing_next.song.title.replaceAll(" ", "%20")}`;
        const components = [
            {
