...
async function handleStaffAdd(interaction: CommandInteraction): Promise<void> {
    const opts = resolveOpts(interaction);
    const userId = opts.find((o: any) => o.name === "user")?.value as string | undefined;
    const roleId = opts.find((o: any) => o.name === "role")?.value as string | undefined;
...
async function handleStaffRemove(interaction: CommandInteraction): Promise<void> {
    const opts = resolveOpts(interaction);
    const userId = opts.find((o: any) => o.name === "user")?.value as string | undefined;
    const roleId = opts.find((o: any) => o.name === "role")?.value as string | undefined;
    ...
    if (userId) {
        ...
        if (idx !== -1) { cfg.staff.userIds.splice(idx, 1); removed.push(`<@${userId}>`); }
    }
    if (roleId) {
        const idx = cfg.staff.roleIds.indexOf(roleId);
        if (idx !== -1) { cfg.staff.roleIds.splice(idx, 1); removed.push(`<@&${roleId}>`); }
    }
...
async function handleLogAdd(interaction: CommandInteraction): Promise<void> {
    const opts    = resolveOpts(interaction);
    const channelId = opts.find((o: any) => o.name === "channel")?.value as string | undefined;
    const guildId   = opts.find((o: any) => o.name === "guild_id")?.value as string | undefined
...
async function handleLogRemove(interaction: CommandInteraction): Promise<void> {
    const opts      = resolveOpts(interaction);
    const channelId = opts.find((o: any) => o.name === "channel")?.value as string | undefined;
...
async function handlePingSet(interaction: CommandInteraction): Promise<void> {
    ...
    const usersRaw = opts.find((o: any) => o.name === "user_ids")?.value as string | undefined;
    ...
    await MessageHandler.success(interaction,
        `Ping targets for ${scope} set to: ${[roleMens, userMens].filter(Boolean).join(" ")}`
    );
}

async function handlePingRemove(interaction: CommandInteraction): Promise<void> {
    const opts       = resolveOpts(interaction);
    const guildIdRaw = opts.find((o: any) => o.name === "guild_id")?.value as string | undefined;
    const guildId    = guildIdRaw === "global" ? undefined
        : guildIdRaw ?? interaction.guild?.id;
