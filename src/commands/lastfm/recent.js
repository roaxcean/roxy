...
import { lfm, trunc, LastfmError } from "./_api.js";
...
export default {
    ...
    async function(interaction) {
        ...
        if (!Array.isArray(tracks))
            tracks = [tracks];
        // Separate the nowplaying track (if any) from the history
        const nowPlayingTrack = tracks.find((t) => t["@attr"]?.nowplaying === "true") ?? null;
        ...
        const lines = [];
        // Show nowplaying at the top with a ▶ marker, no timestamp
        if (nowPlayingTrack) {
