...
export async function buildGroupsPage(
    ...
    async function(interaction: CommandInteraction) {
        ...
        try {
            const data = await rbx<{ data: any[] }>(
                `https://groups.roblox.com/v1/users/${user.id}/groups/roles?includeLocked=false`
            );
        ...
        robloxCache.set<GroupsCacheEntry>(`groups:${user.id}`, { user, memberships });
