...
async function resolveUniverseId(input: string): Promise<number> {
    // if its already a universe ID (long number), use directly
    ...
    try {
        const data = await rbx<{ data: any[] }>(`https://games.roblox.com/v1/games?universeIds=${id}`);
        if (data.data?.length) return id;
    } catch { /* empty */ }

    // fall back: treat as place ID, resolve to universe
    const universeData = await rbx<{ universeId: number }>(
...
export default {
    ...
    async function(interaction: CommandInteraction) {
        const input = getOption<string>(interaction, "id")!;
        ...
        try {
            ...
            game = data.data?.[0];
            if (!game) throw new RobloxError("Game not found.");
        } catch (err) {
            await MessageHandler.error(interaction, err instanceof RobloxError ? err.message : String(err));
            return;
        }
