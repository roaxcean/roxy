...
async function getGroupThumbnail(groupId: number): Promise<string | null> {
    try {
...
export async function buildGroupGamesPage(
    group: any,
    games: any[],
    ...
    const slice = games.slice(
        ...
        safePage * GAMES_PER_PAGE + GAMES_PER_PAGE,
    );

    const lines = slice.map((g: any, i: number) => {
...
export default {
    ...
    async function(interaction: CommandInteraction) {
        const idInput = getOption<string>(interaction, "id")!.trim();
        const view    = getOption<string>(interaction, "view") ?? "overview";
        ...
        const statsMap = new Map<number, { visits: number; placeId: number | null }>();
        try {
            const statsData = await rbx<{ data: any[] }>(
        ...
        robloxCache.set<GroupGamesCacheEntry>(`groupgames:${groupId}`, { group, games, statsMap });
