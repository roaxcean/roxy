...
import { Command } from "../sys/types.js";
...
function sortCategories(entries: [string, Command[]][]) {
...
function buildCategoryBlock(category: string, cmds: Command[]): string | null {
    const lines: string[] = [];

    for (const cmd of cmds) {
        const subs = cmd.options?.filter(
            (o: any) => o.type === Constants.ApplicationCommandOptionTypes.SUB_COMMAND
        ) ?? [];
 ...
 */
export async function buildHelpPage(page: number): Promise<{
    components: any[];
    page: number;
    totalPages: number;
}> {
    ...
    const slice = blocks.slice(
        ...
        safePage * CATEGORIES_PER_PAGE + CATEGORIES_PER_PAGE
    );
