...
import { setActivity, ActivityType } from "../../sys/presence.js";
...
export default {
    ...
    async function(interaction: CommandInteraction) {
        const type = getOption<ActivityType>(interaction, "type")!;
        const text = getOption<string>(interaction, "text")!.trim();
        const url  = getOption<string>(interaction, "url")?.trim();
        ...
        if (type === "streaming" && url) {
            ...
            if (!validUrl) {
                await MessageHandler.warning(
                    interaction,
                    "Streaming URL must be a Twitch or YouTube link — Discord only shows the status as streaming for those."
                );
                return;
            }
        }
