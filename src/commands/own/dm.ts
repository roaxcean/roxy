...
const DM_ERROR_REASONS: Record<number, string> = {
...
function friendlyDmError(err: any): string {
...
function parseUserIds(raw: string): { ids: string[]; invalid: string[] } {
...
async function dmUser(userId: string, payload: object): Promise<DmResult> {
    try {
...
export default {
    ...
    async function(interaction: CommandInteraction) {
        ...
        const pickedUserId  = opts.find(o => o.name === "user")?.value as string | undefined;
        const bulkRaw       = opts.find(o => o.name === "user_ids")?.value as string | undefined;
        ...
        if (!pickedUserId && !bulkRaw) {
            await MessageHandler.warning(
                interaction,
                "Provide at least one target: `user` (picker) or `user_ids` (bulk string).",
                true
            );
            return;
        }
        ...
        if (targetIds.size === 0) {
            await MessageHandler.error(
                interaction,
                "No valid user IDs could be parsed from `user_ids`.",
                parseErrors.length
        ...
        const results = await Promise.all(
            [...targetIds].map(id => dmUser(id, payload))
        );
        ...
        if (succeeded.length) {
            lines.push(
                `**${succeeded.length} sent** — ${succeeded.map(r => `<@${r.userId}>`).join(", ")}`
            );
        }
        ...
        if (failed.length) {
            lines.push(
                ...
                ...failed.map(r => `> <@${r.userId}> — ${r.reason}`)
            );
        }

        if (parseErrors.length) {
            lines.push(
                `**${parseErrors.length} skipped** (not valid IDs) — ${parseErrors.map(e => `\`${e}\``).join(", ")}`
            );
        }
