...
export default {
    ...
    async function(interaction: CommandInteraction) {
        ...
        const opts: any[] =
            raw?.[0]?.options ??   // subcommand invocation
            raw ??                 // top-level invocation
            [];

        const componentsRaw = opts.find(o => o.name === "components")?.value as string | undefined;
        const channelId     = opts.find(o => o.name === "channel")?.value as string | undefined;
        const messageId     = opts.find(o => o.name === "message_id")?.value as string | undefined;
        ...
        try {
            ...
            if (!Array.isArray(parsed)) {
                await MessageHandler.error(
                    interaction,
                    "JSON must be an array of top-level component objects.",
                    undefined,
                    true
                );
                return;
        ...
        try {
            if (messageId) {
                ...
                await MessageHandler.success(
                    interaction,
                    `Message [\`${messageId}\`](https://discord.com/channels/${interaction.guild!.id ?? "@me"}/${channelId}/${messageId}) updated.`
                );
            } else {
                const sent = await app.createMessage(channelId, payload);
                await MessageHandler.success(
                    interaction,
                    `Message sent <:arrowr:1426686528276529313> [**jump**](https://discord.com/channels/${interaction.guild!.id ?? "@me"}/${channelId}/${sent.id})`
                );
            }
        } catch (err: any) {
            await MessageHandler.error(
